...
import icon.common.render_utils as util
from icon.dataset.mesh_util import get_visibility
import torch
import numpy as np
...
class Render:
    ...
    def VF2Mesh(self, verts, faces, rgbs=None):
    ...
    def load_meshes(self, verts, faces, rgbs=None):
        ...
        if isinstance(verts, list):
            self.meshes = []
            for V, F, C in zip(verts, faces, rgbs):
                self.meshes.append(self.VF2Mesh(V, F, C))
        else:
            self.meshes = [self.VF2Mesh(verts, faces, rgbs)]
    ...
    def get_rgb_image(self, cam_ids=[0, 2], bg='gray'):
        ...
        for cam_id in range(len(self.cam_pos)):
            if cam_id in cam_ids:
                self.init_renderer(self.get_camera(cam_id), "ori_mesh", bg)
                if len(cam_ids) == 4:
                    rendered_img = (
    ...
    def get_rendered_video(self, images, save_path):
        ...
        new_shape = np.around((self.size / old_shape[0]) * old_shape).astype(np.int32)
