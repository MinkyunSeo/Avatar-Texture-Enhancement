...
class ResBlkPIFuNet(BasePIFuNet):
    ...
    def query(self, points, calibs, transforms=None, labels=None):
        ...
        xyz = self.projection(points, calibs, transforms)   # [B*P, 2, N]
        xy = xyz[:, :2, :]
        z = xyz[:, 2:3, :]

        z_feat = self.normalizer(z)

        # This is a li st of [B, Feat_i, N] features
    ...
    def get_3D_SSIM_error(self):
        ## caculate SSIM error

        ## self.preds: [B, P, Res, N]
        ## self.labels: [B, P, Res, N]
        patch = self.preds
        patch_gt = self.labels
    ...
    def forward(self, images, im_feat, points, calibs, transforms=None, labels=None):
        ...
        Args:
            ...
            points: [B, 3, P, N]
        """
        if self.opt.patch_sample is True:
            B, _, P, N = points.shape
            points = points.reshape(B, -1, P*N)
        ...
        if self.opt.patch_sample is True:
            # res = res.reshape(B, P, -1, N)
            # self.preds = self.preds.reshape(B, P, -1, N)
            res = res.reshape(B, -1, P, N).permute(0, 2, 1, 3)
            self.preds = self.preds.reshape(B, -1, P, N).permute(0, 2, 1, 3)
            self.labels = self.labels.permute(0, 2, 1, 3)

        if self.opt.use_3D_SSIM:
            error = self.get_3D_SSIM_error()
        else:
