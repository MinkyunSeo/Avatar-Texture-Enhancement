...
def train_color(opt):
    ...
    if opt.load_netG_checkpoint_path is not None:
        ...
        raise NotImplementedError('Model path is not assigned')
    ...
    if opt.continue_train:
        raise NotImplementedError('Continue train is not implemented')

    os.makedirs(f"{opt.results_path}/{opt.name}/ckpts/", exist_ok=True)
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if train_idx % opt.freq_save == 0 and train_idx != 0:
                checkpoint_dir = f"{opt.results_path}/{opt.name}/ckpts/"
                torch.save(netC.state_dict(), os.path.join(checkpoint_dir, 'netC_latest'))
                torch.save(netC.state_dict(), os.path.join(checkpoint_dir, f'netC_epoch_{epoch}'))

            if train_idx % opt.freq_save_ply == 0:
                save_path = '%s/%s/pred_col.ply' % (opt.results_path, opt.name)
                if opt.patch_sample:
                    B, _, P, N = color_sample_tensor.shape
                    points = color_sample_tensor[0, :, int(P/2)].transpose(0, 1).cpu()
                    rgb = resC[0][int(P/2)].transpose(0, 1).cpu()
                else:
